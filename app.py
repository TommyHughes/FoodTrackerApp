...
def connect_db():
    sql = sqlite3.connect('C:\\Users\\thughes\\programming\\Misc\\Python\\foodtrackerapp\\food_log.db')
...
def view(date):
    ...
    log_cur = db.execute('select food.name, food.protein, food.carbohydrates, food.fat, food.calories from log_date left join food_date on food_date.log_date_id = log_date.id left join food on food.id = food_date.food_id where log_date.entry_date = ?',[date])
    ...
    for f in log_results:
        ...
        totals['carbohydrates'] += f['carbohydrates']
        totals['fat'] += f['fat']
